...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Cloud Attenuation Exceedance Calculator",
    ...
    fs.add_argument("--o_name", dest = "o_name", action = "store", type = str, default='Cloud_Attenuation_Exceedance' , help = "Output File Base Name")
    ...
    unavailabilities = np.logspace(-1.1, 1.8, 500)
    #unavailabilities = np.linspace(0.05, 70, 500)

    for el in elevations:
        print("Computing Cloud Attenuation for Elevation = {:2.1f}".format(el))
        A_g, A_c, A_r, A_s, A_t = [], [], [], [], []
    ...
    for el in elevations:
        print("Plotting Cloud Attenuation for Elevation = {:2.1f}".format(el))
        lbl_str='El={:2.1f}'.format(el)
        ax.semilogx(unavailabilities, cloud_attn["{:2.1f}".format(el)], label=lbl_str+"$^{\circ}$")
    ...
    title_str = 'Cloud Attenuation Exceedance'
    ...
    ax.text(0.05, 0.9, subtitle_str, transform=ax.transAxes, fontsize=10,
